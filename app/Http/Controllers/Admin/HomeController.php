...
class HomeController extends Controller
{
    public function index()
    {
        ...
        $earning            = 0;
        $user               = User::where('tenant_id', tenant('id'))->where('type', '!=', 'Agency')->where('created_by', Auth::user()->id)->count();
        ...
        $documentsDatas = collect();
        $posts          = collect();
        $events         = Event::latest()->take(5)->get();
        return view('admin.dashboard.home', compact('user', 'role', 'planExpiredDate', 'earning', 'paymentTypes', 'documents', 'supports', 'documentsDatas', 'posts', 'events'));
    }
