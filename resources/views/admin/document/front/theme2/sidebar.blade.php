            ...
            <div class="main-logo doc-logo mobile-device">
                <a href="javascript:void(0);" class="logo-space" tabindex="0">
                    {!! Form::image(
                        ...
                        null,
                        [
                            'class' => 'wid-50 rounded-circle me-3 wid-logo side-bar-logo',
                        ],
                    ) !!}
                </a>
            </div>
...
@if ($key == 0) active-color @endif
                                @endif
                            @foreach ($documentMenu as $key => $docsMenu)
                            ...
                            <span class="acnav-label @if (request()->is('document/' . $menu->slug . '/' . $menus->slug . '/*')) is-open @endif">
                                <a href="{{ route('document.menu.menu', $menu->slug) }}" tabindex="0">
                            ...
                            <ul class="acnav-list">
                                @foreach ($documentMenu as $key => $docsMenu)
                                    <li class="@if (Request::is('document/' . $menu->slug . '/' . $docsMenu->slug)) activemenu @endif">
                                        <a class="active" tabindex="0"
                                            href="{{ route('document.sub.menu', [$menu->slug, $docsMenu->slug]) }}">
                                            {{ substr($docsMenu->title, 0, 20) . (strlen($docsMenu->title) > 20 ? '...' : '') }}
                                        </a>
                                    </li>
                                @endforeach
                            </ul>
...
@if ($key == 0) activemenu @endif @endif">
                            <a class="acnav-label" href="{{ route('document.menu.menu', $menu->slug) }}">
                ...
                <li class="menu-item has-children">
                    ...
                    <ul class="dropdown-menu acnav-list">
                        @foreach ($changeLogJsons as $changeLog)
                            ...
                            <li class="c_log">
                                <a class="nav-link"
                                    href="{{ $lastSegments != 'changelog' ? route('document.menu.menu', [$lastSegment, 'changelog']) . '#' . $changeLogId : '#' . $changeLogId }}">
                                    {{ 'v' . $changeLog['version'] }}
                                </a>
                            </li>
                        @endforeach
